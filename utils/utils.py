...
def orthogonalize_weights(tensor, act):
    ...
    )  # @mst: avoid modifying the original tensor
    ...
    q, r = torch.qr(
        flattened)  # q: (rows, cols), r: (cols, cols) if rows > cols.
...
def get_n_params(model):
    total = sum([
        param.nelement() if param.requires_grad else 0
...
def get_n_params_(model):
    ...
    for _, module in model.named_modules():
        if isinstance(module, nn.Conv2d) or isinstance(
                module, nn.Linear):  # only consider Conv2d and Linear, no BN
...
def get_n_flops(model=None, input_res=224, multiply_adds=True, n_channel=3):
    ...
    def conv_hook(self, input, output):
        ...
        )  # @mst: this should be considering the pruned model
    ...
    foo(model)
    input = Variable(torch.rand(n_channel, input_res, input_res).unsqueeze(0),
                     requires_grad=True)
    out = model(input)

    total_flops = (
        sum(list_conv) + sum(list_linear)
    )  # + sum(list_bn) + sum(list_relu) + sum(list_pooling) + sum(list_upsample))
    total_flops /= 1e9
...
def get_n_flops_(model=None,
    ...
    def conv_hook(self, input, output):
        flops = np.prod(self.weight.data.shape) * output.size(2) * output.size(
            3) / self.groups
        list_conv.append(flops)
...
class PresetLRScheduler(object):
    ...
    def __init__(self, decay_schedule):
        ...
        self.decay_schedule = {}
        for k, v in decay_schedule.items(
        ):  # a dict, example: {"0":0.001, "30":0.00001, "45":0.000001}
            self.decay_schedule[int(
                float(k))] = v  # to float first in case of '1e3'
...
def kronecker(A, B):
    return torch.einsum("ab,cd->acbd", A,
                        B).view(A.size(0) * B.size(0),
                                A.size(1) * B.size(1))
...
def kd_loss(student_scores, teacher_scores, temp=1, weights=None):
    ...
    if isinstance(weights, type(None)):
        ...
        )  # 2020-06-21 @mst: Since 'size_average' is deprecated, use 'reduction' instead.
    else:
...
def test(net, test_loader):
    ...
    for c in range(n_class):
        acc_test[c] = 0 if cnt_test[c] == 0 else acc_test[c] / float(
            cnt_test[c])
    ...
    def __call__(self, name, x):
        ...
        if self.mu > 0:
            ...
            if name in self.history.keys():
                new_average = self.mu * self.history[name] + (
                    1.0 - self.mu) * x.clone()
            else:
                new_average = x.clone()
...
colors = [
    "gray", "blue", "black", "yellow", "green", "yellowgreen", "gold",
...
def smart_weights_load(net, w_path, key=None, load_mode='exact'):
    ...
    '''
    common_weights_keys = [
        'T', 'S', 'G', 'model', 'state_dict', 'state_dict_t'
    ]
    ...
    if isinstance(ckpt, OrderedDict):
        ...
        if key:
            ...
            intersection = [
                k for k in ckpt.keys() if k in common_weights_keys
                and isinstance(ckpt[k], OrderedDict)
            ]
            if len(intersection) == 1:
                ...
                print(
                    'Error: multiple or no model keys found in ckpt: %s. Please explicitly appoint one'
                    % intersection)
                exit(1)
        ...
        for name, m in net.named_modules():
            if name:
                ...
                if matched_param_name:
                    ...
                    print("net module name: '%s' <- '%s' (ckpt module name)" %
                          (name, matched_param_name))
                else:
                    print(
                        "Error: cannot find matched module in ckpt for module '%s' in net. Please check manually."
                        % name)
                    exit(1)
...
def get_total_index_by_learnable_index(net, learnable_index):
    ...
    '''
    layer_type_considered = [
        nn.Conv2d, nn.ReLU, nn.LeakyReLU, nn.PReLU, nn.BatchNorm2d,
        nn.MaxPool2d, nn.AvgPool2d, nn.Linear
    ]
    cnt_total = -1
    cnt_learnable = -1
    for _, m in net.named_modules():
...
class Timer():
    ...
    def predict_finish_time(self, ave_window=3):
        ...
        return finish_t + ' (speed: %.2fs per timing, total_time: %s)' % (
            sec_per_epoch, total_t)
...
class AccuracyManager():
    ...
    def update(self, time, acc1, acc5=None):
        acc = pd.DataFrame([[time, acc1, acc5]],
                           columns=['time', 'acc1',
                                    'acc5'])  # time can be epoch or step
...
def format_acc_log(acc1_set, lr, acc5=None, time_unit='Epoch'):
    ...
    if acc5:
        line = 'Acc1 %.4f Acc5 %.4f @ %s %d (Best_Acc1 %.4f @ %s %d) LR %s' % (
            acc1, acc5, time_unit, acc1_time, acc1_best, time_unit,
            acc1_best_time, lr)
    else:
        line = 'Acc1 %.4f @ %s %d (Best_Acc1 %.4f @ %s %d) LR %s' % (
            acc1, time_unit, acc1_time, acc1_best, time_unit, acc1_best_time,
            lr)
    return line
...
def visualize_filter(layer,
    ...
    if pick_mode == 'rand':
        filter_ix = np.random.permutation(
            n)[:n_filter_plot]  # filter indexes to plot
        channel_ix = np.random.permutation(
            c)[:n_channel_plot]  # channel indexes to plot
    else:
    ...
    for i in filter_ix:
        ...
        for j in channel_ix:
            ...
            cax = divider.append_axes('right', size=0.05, pad=0.05)
            fig.colorbar(im, cax=cax, orientation='vertical')
            save_path = '%s/filter_visualize__%s__layer%s__filter%s__channel%s' % (
                save_dir, prefix, layer_id, i, j)
            fig.savefig(save_path + ext, bbox_inches='tight')
            plt.close(fig)


def visualize_feature_map(fm,
                          layer_id,
                          save_dir,
                          n_channel_plot=16,
                          pick_mode='rand',
    ...
    )[0]  # shape: [N, C, H, W], N is batch size. Default: batch size should be 1
    if plot_abs:
        fm = np.abs(fm)
    c = fm.shape[0]
    n_channel_plot = min(n_channel_plot, c)
    ...
    for j in channel_ix:
        ...
        cax = divider.append_axes('right', size=0.05, pad=0.05)
        fig.colorbar(im, cax=cax, orientation='vertical')
        save_path = '%s/featmap_visualization__%s__layer%s__channel%s' % (
            save_dir, prefix, layer_id, j)
        fig.savefig(save_path + ext, bbox_inches='tight')
        plt.close(fig)
...
def add_noise_to_model(model, std=0.01):
    ...
    for name, module in model.named_modules():
        if isinstance(
                module,
            (nn.Conv2d, nn.Linear,
             nn.BatchNorm2d)):  # all learnable params for a typical DNN
            w = module.weight
...
def get_jacobian_singular_values(model,
                                 data_loader,
                                 num_classes,
                                 n_loop=20,
                                 print_func=print,
    ...
    for i, (images, target) in enumerate(data_loader):
        if i in picked_batch:
            ...
            jacobian = compute_jacobian(
                images, output
            )  # shape [batch_size, num_classes, num_channels, input_width, input_height]
            jacobian = jacobian.view(
                batch_size, num_classes, -1
            )  # shape [batch_size, num_classes, num_channels*input_width*input_height]
            u, s, v = torch.svd(
                jacobian
            )  # u: [batch_size, num_channels*input_width*input_height, num_classes], s: [batch_size, num_classes], v: [batch_size, num_channels*input_width*input_height, num_classes]
            s = s.data.cpu().numpy()
            jsv.append(s)
            condition_number.append(s.max(axis=1) / s.min(axis=1))
            print_func('[%3d/%3d] calculating Jacobian...' %
                       (i, len(data_loader)))
...
def approximate_entropy(X, num_bins=10, esp=1e-30):
    ...
    for di in range(X.shape[1]):
        ...
        prob = np.histogram(samples, bins=bins,
                            density=False)[0] / len(samples)
        entropy.append(
            (-np.log2(prob + esp) *
             prob).sum())  # esp for numerical stability when prob = 0
        ...
        if '.' in k:  # @mst-TODO: hardcode pattern, may be risky
            ...
            if arg_dict[f'{module}.ON']:  # this module is being used
                if not hasattr(args, module):
                    args.__setattr__(module,
                                     EmptyClass())  # set to a blank class
                args.__dict__[module].__dict__[
                    arg] = v  # args.'deepmixup.depth' = 10 --> args.deepmixup.depth = 10
            args.__delattr__(k)
    return args
